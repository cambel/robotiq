...
class CModelActionController(object):
  ...
  def _status_cb(self, msg):
    ...
    js_msg.name.append('hande_joint_finger')
    # js_msg.position.append(0.8*self._status.gPO/self._min_gap_counts)
    js_msg.position.append(0.026*self._status.gPO/self._min_gap_counts)
    self.js_pub.publish(js_msg)
    js_msg.name = []
    js_msg.name.append(self._gripper_prefix + 'joint_finger')
